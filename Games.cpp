...
int WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, PSTR lpCmdLine, int nCmdShow) {
	...
	//cube.init(dx);
	//gem.init(dx, "Resources/acacia_003.gem");
	...
	//generateForestFile("forest.txt", 1000, 1000, 5.0f, 50);
	...
	while (true)
	{
		if (currentMode == DEFERRED) {
			...
			//pine.draw(&shader3, dx, text, camera);
			//pine1.draw(&shader2, dx, text, camera);
			for (int i = 0; i < 500; i++) {
